...
def writefilev2(fileName, mode):
    global chipid
    with open("write.log", 'w') as log_file: 
        bytes = open(fileName, 'rb').read()
        if mode == mode_write_v2:
            print('Filesize: ' + str(len(bytes)) + ' bytes')
            print('Filesize: ' + str(len(bytes)) + ' bytes', file=log_file)
        if len(bytes) < 256:
            print('Something wrong with the bin file', file=log_file)
        ...
        while curr_addr < len(bytes):
            ...
            outbuffer[5] = 0x00
            outbuffer[6] = 0x00
            # outbuffer[7] = i & 0xff       # is it really necessary ???
            outbuffer[7] = 0x00             # flashes ok with byte sevet set to 0x00
            for x in range(pkt_length):
